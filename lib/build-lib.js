// Pure HTML-building helpers for the static-site build (build.js). Extracted
// from build.js so the markup generation, escaping, and marker replacement can
// be unit-tested without filesystem side effects.
import { eventBadgeLabel } from './dates.js';
...
export const NOTICE_ICONS = { info: 'ℹ️', warning: '⚠️', urgent: '🚨' };
...
// Returns { html, active, expired } so the build can also report status.
export function buildNoticeHTML(notice, now = new Date()) {
  const expired = !!(notice.expires && new Date(notice.expires) <= now);
  const active = !!(notice.active && notice.message && !expired);
  if (!active) return { html: '', active: false, expired };
  const html = `<div class="notice-bar notice-${notice.type}" id="notice-bar"${notice.expires ? ` data-expires="${escapeHTML(notice.expires)}"` : ''}>
    <div class="notice-bar-inner">
      <span>${NOTICE_ICONS[notice.type] || 'ℹ️'}</span>
      <span>${escapeHTML(notice.message)}</span>
      ${notice.dismissible ? '<button class="notice-bar-close" onclick="dismissNotice()" aria-label="Dismiss">✕</button>' : ''}
    </div>
  </div>`;
  return { html, active: true, expired: false };
}
...
// Replace a marked region of text, throwing if the marker is absent so the
// build fails loudly instead of silently shipping an unpatched template.
export function replaceRegion(text, re, replacement, markerName) {
  if (!re.test(text)) throw new Error(`${markerName} marker(s) missing`);
