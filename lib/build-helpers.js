// Pure HTML-building helpers used by build.js. Kept free of file/process
...
export function buildUpdateCard(u, now = new Date()) {
  ...
  let badge = '';
  if (u.eventDate) {
    const rel = relativeDayLabel(u.eventDate, now);
...
// ── Build filter buttons from unique categories in updates ──
...
export function buildNoticeBar(notice, now = new Date()) {
  if (!noticeState(notice, now).active) return '';
  return `<div class="notice-bar notice-${notice.type}" id="notice-bar"${notice.expires ? ` data-expires="${escapeHTML(notice.expires)}"` : ''}>
